...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        self.section_rows: Dict[str, Dict[str, Dict[str, object]]] = {}
        ...
        self.section_rows = {
            ...
            if (filtered := self.ignore_samples(rows))
        }
    ...
    def _collect_section(self, section: str) -> Dict[str, Dict[str, object]]:
    ...
    def _add_general_stats(self) -> None:
        ...
        for sample, row in demux.items():
            ...
            for key in ("observed_outputs", "missing_required_outputs", "matched_reads", "failed_reads"):
                if key in row:
                    gs[sample][key] = to_number(row[key])
        ...
        headers: Dict[str, ColumnDict] = {
            "observed_outputs": {"title": "Ultima Observed Outputs", "description": "Observed required and optional output files"},
            "missing_required_outputs": {"title": "Ultima Missing Required", "description": "Required Ultima output files missing from run evidence"},
            "matched_reads": {"title": "Ultima Matched Reads", "description": "Matched or demultiplexed reads when reported"},
            "failed_reads": {"title": "Ultima Failed Reads", "description": "Failed reads when reported"},
            "mean_depth": {"title": "Ultima Mean Depth", "suffix": "x", "description": "Mean depth from normalized Ultima coverage outputs"},
            "median_depth": {"title": "Ultima Median Depth", "suffix": "x", "description": "Median depth from normalized Ultima coverage outputs"},
            ...
            "PCT_contamination": {"title": "Ultima Contam %", "suffix": "%", "description": "Percent contamination from selfSM contamination stats"},
            "contamination_fraction": {"title": "Ultima Contam Frac", "description": "Contamination fraction from normalized output"},
        }
    ...
    def _section_table(self, section: str, rows: Mapping[str, Mapping[str, object]]):
        headers = table_headers(rows)
        return table.plot(rows, headers, {"id": f"ultima_{section}", "title": SECTION_NAMES[section]})

    def _section_bargraph(self, section: str, rows: Mapping[str, Mapping[str, object]]):
...
def bargraph_keys(section: str, rows: Mapping[str, Mapping[str, object]]) -> List[str]:
    preferred = {

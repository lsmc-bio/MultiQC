...
_current_group: ContextVar[Optional[str]] = ContextVar("_current_group", default=None)
...
def section(name: str) -> Iterator[None]:
    ...
    """
    section_token = _current_section.set(name)
    # Entering a new section clears any group tag so sibling blocks can't leak.
    group_token = _current_group.set(None)
    try:
        yield
    finally:
        _current_group.reset(group_token)
        _current_section.reset(section_token)

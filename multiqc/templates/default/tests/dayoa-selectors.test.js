...
test("selector values cycle neutral to include to exclude to neutral", () => {
  ...
  const key = dayoaIdentityKey(records[0], "library");

  assert.equal(dayoaSelectorValueState(state, "library", key), "neutral");
  assert.equal(cycleDayoaSelectorValue(state, "library", key), "include");
  assert.equal(cycleDayoaSelectorValue(state, "library", key), "exclude");
  assert.equal(cycleDayoaSelectorValue(state, "library", key), "neutral");
...
test("includes are OR within a dimension and AND across dimensions", () => {
  ...
  state.included.library.add(dayoaIdentityKey(records[0], "library"));
  state.included.library.add(dayoaIdentityKey(records[1], "library"));
...
test("exclusions form a negative set and override matching includes", () => {
  const state = createDayoaSelectorState();
  state.included.specimen.add(dayoaIdentityKey(records[0], "specimen"));
  state.excluded.library.add(dayoaIdentityKey(records[1], "library"));
